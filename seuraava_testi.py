...
def add_signature_to_node(data, *args):
          print("ADDING signature to node (node + ts_created) :",data)
          node_address, ts_created = data
          global daily_tx_counter
          extra_data = steal_node_info(data)

          stake_value,node_weight = extra_data
          print("We came back from steal node info with data:",data)
          print("And we will now go and add 1st signer to database")
          first_block=""
          total_blocks=0
          con = sqlite3.connect('cellframe.db')
          with con:
            ...
            query = """ SELECT id FROM {table} ORDER BY id DESC LIMIT 1""".format(table=node_address) ##GET THE BIGGEST ROW ID
            cur.execute(query)
            result = cur.fetchone()
            row_id, = result
   #         print("ROW:",row_id)

            query = """ SELECT blocks FROM {table} ORDER BY id DESC LIMIT 1""".format(table=node_address) ##lets see if this is the 1st block for this node
            cur.execute(query)
            result = cur.fetchone() #grab 1 line
    ...
    #        print("Blocks :",blocks)
            ...
            query = """ SELECT all_time_blocks FROM {table} ORDER BY id DESC LIMIT 1""".format(table=node_address) ##lets grab all time blocks as well
            cur.execute(query)
            result = cur.fetchone() #grab 1 line
            all_time_blocks,=result
            print("But before that we will check if day has changed by comparing ts_created with the last hash date in DB")
            info = has_day_changed(ts_created)

            if info == 1: #new day
                   row_id += 1
                   blocks = 1 #new day, first block
                   all_time_blocks += 1
          #         print("All time blocks after has day changed...",all_time_blocks)
                   print("Add signature to node noticed day has changed...")
                   input("...going to UPDATE into next id blocks,all_time_blocks,weight and stake value")
                   query = """ UPDATE {table} SET blocks=?, all_time_blocks=?, weight=?, stake_value=? WHERE id=?""".format(table=node_address)
                   tieto=(blocks,all_time_blocks,node_weight,stake_value,row_id)
                   cur.execute(query, tieto)
                   con.commit()
                   input("...done. Let's list keys once more to collect network weight")
...
#                   print("list item length :",command_length)     #### every new day we need to check network total weight
 #                  print("last items :",command[command_length-3])
                   network_weight = command[command_length-2]
                   network_weight = network_weight[14:].rsplit()
                   print("Network weight :", network_weight[0])
                   input("**************** CHECK THIS *************************")
                   ...
                   with con:
                        cur = con.cursor()
                        query = """SELECT daily_transactions,cumulative_transactions FROM {table} ORDER BY id DESC LIMIT 1""".format(table="transactions")
                        cur.execute(query)
                        data=cur.fetchone()

                        last_daily_tx, new_daily_tx = data
                        cumul_transactions = last_daily_tx + new_daily_tx


                        query = """INSERT INTO {table} VALUES (NULL,?,?,?,?)""".format(table="transactions")
                        tieto=(ts_created,1,cumul_transactions,network_weight[0])
                        cur.execute(query, tieto)
                        con.commit()
  #                      print("Daily transactions : ",1) #set first transaction for this day
                        daily_tx_counter = 1
                   return

            if info == 0: #SAME OLD DAY

                   blocks += 1 #add one more block to this node
                   all_time_blocks += 1
                   print("Add signature no node says day has NOT changed...")
                   print("...going to UPDATE blocks and all time blocks info")
                   query=""" UPDATE {table} SET blocks=? WHERE id=?""".format(table=node_address)
                   tieto=(blocks, row_id)
                   cur.execute(query, tieto)

                   query=""" UPDATE {table} SET all_time_blocks=? WHERE id=?""".format(table=node_address)
                   tieto = (all_time_blocks, row_id)
                   cur.execute(query, tieto)
                   con.commit()

                   con = sqlite3.connect('cellframe.db')
                   with con:
                        daily_tx_counter = daily_tx_counter+1
                        cur = con.cursor()
                        query = """ SELECT id, daily_transactions FROM transactions ORDER BY id DESC LIMIT 1"""
                        cur.execute(query)
                        con.commit()
                        id, daily_tx_counter, = cur.fetchone()
 #                       print("daily tx counter =",daily_tx_counter)
                        daily_tx_counter = int(daily_tx_counter)+1
                        query = """ UPDATE {table} SET daily_transactions=? WHERE id=?""".format(table="transactions")
                        cur.execute(query,[daily_tx_counter, id])
                        con.commit()
   #                     input("stop here")
                   return
